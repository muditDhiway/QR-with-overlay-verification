...
//     let original_image = image::open("qr_with_separator_overlay.png")
//         .expect("Failed to open the QR code image");
    
//     // Resize the QR code image to a larger size
//     let resized_image = resize_image(&original_image, 1000, 1000); // Resize to 1000x1000 pixels

//     let decoder = bardecoder::default_decoder();
    
//     let results = decoder.decode(&resized_image);

//     let message: Vec<String> = results.iter().map(|r| r.as_ref().unwrap().clone()).collect();
//     println!("Decoded message: {:?}", message[0]);
...
//             match result {
//                 Ok(_content) => println!("QR successfully decoded from image!"),
//                 Err(e) => println!("Failed to decode QR code: {}", e),
//             }
//         }
//     }

//     // Start working on the actual image
//     let img_gray = original_image.into_luma8();

//     let (mut width, height) = (img_gray.width(), img_gray.height());
//     println!("width: {}, height: {}", width, height);

//     if width != height || width < 21 {
//         println!("image resolution in incorrect");
//     }

//     let mut hasher = Sha256::new();
//     hasher.update(message[0].as_bytes());
...
//     println!("Reference bits: {:?}\n", reference_bits);

//     let finder_size = 7; // Finder pattern is always 7x7 modules

//     width -= 1;

//     let extract_bits_from_separator =
//         |x_start: u32, y_start: u32, x_stop: u32, y_stop: u32, direction: &str| -> Vec<u8> {
...
//         "horizontal",
//     ));
//     println!("Overlay bits: {:?}", overlay_bits);

//     // Compare the extracted bits with the reference bits
//     let is_valid = overlay_bits == reference_bits;

//     print!("verification: {}", if is_valid { "valid" } else { "invalid" });
// }
...
fn main() {
    ...
    let image_path = "qr_with_separator_overlay.png";
